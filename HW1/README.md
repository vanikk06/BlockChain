...
在研究不同Dapp時剛好看到一篇[文章](https://www.blocktempo.com/blockchain-with-or-withour-token/)，它解釋了Token在區塊鏈上背後蘊含的價值是什麼，裡面提到區塊鏈的本質──資料透明不可逆的記帳過程，讓人類社會中不論有形或無形的價值都可以紀錄在區塊鏈中，解決原本價值存在資訊不對稱和無法公平定價的問題。

音樂產業因應網絡時代發展數位音樂慢慢成為主流，但數位音樂因為取得太方便，導致音樂的版權、價值無法有效的管理，然而透過將數位音樂上鏈的方式，公開透明、不可竄改的特性可以讓版權的管理更加容易，發行限量Token控制音樂的價值流動，可以有效的保存數位音樂的價值，讓無形的價值可以更好的保存、交易。
